...
class HistoriaClinicaController extends Controller {
    ...
    public function exportarHistoriaAction($id) {
        $em = $this->getDoctrine()->getManager();
        $grupos = $em->getRepository('HistClinicaBundle:Grupo')->findGruposOrdenados();
        $paciente = $em->getRepository('HistClinicaBundle:Paciente')->find($id);
        ...
        return $this->render('HistClinicaBundle:HistoriaClinica:exportar.html.twig', array(
                    'grupos' => $grupos,
                    'paciente' => $paciente,
                    'respuestas' => $totalrtas,
                    'historiaClinica' => $historiaClinica,
    ...
    public function llamarExportarAction($id) {
        $pageUrl = $this->generateUrl('exportar_historia', array('id'=>$id), true); // use absolute path!
        //$em = $this->getDoctrine()->getManager();
        //$paciente = $em->getRepository('HistClinicaBundle:Paciente')->find($id);
        return new Response(
                $this->get('knp_snappy.pdf')->getOutput($pageUrl), 200, array(
            'Content-Type' => 'application/pdf',
            'Content-Disposition' => 'attachment; filename="Dental360.pdf"'
                )
        );
    }
